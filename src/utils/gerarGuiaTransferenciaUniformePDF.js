...
export function gerarGuiaTransferenciaUniformePDF(dadosEnvio) {
  ...
  doc.setFontSize(11);
  doc.text('BIG MASTER SUPERMERCADOS • LOGÍSTICA & ESTOQUE', margin + 18, currentY + 7);

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(148, 163, 184);
  doc.text('GUIA DE TRANSFERÊNCIA E REMESSA DE UNIFORMES EM MASSA', margin + 18, currentY + 13);

  const dataAtual = new Date().toLocaleString('pt-BR');
  doc.setFontSize(7.5);
  doc.text(`Data: ${dataAtual}`, pageWidth - margin - 4, currentY + 10, { align: 'right' });
